...
class MapBuilder(Rasterizer):
    ...
    def rasterize(
        ...
        obj_im = self.render_objective_map(center_in_world_m, raster_from_world, history_tl_faces[0])
    ...
    def render_objective_map(
        ...
        img = 255 * np.ones(shape=(self.raster_size[1], self.raster_size[0], 3), dtype=np.uint8)
        obj_map = np.zeros(shape=(self.raster_size[1], self.raster_size[0]), dtype=np.float32)
        ...
        agt_im = np.concatenate((agents_images, ego_images), -1)
