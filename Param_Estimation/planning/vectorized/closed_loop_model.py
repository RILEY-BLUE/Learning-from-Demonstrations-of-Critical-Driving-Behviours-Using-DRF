...
class VectorizedUnrollModel(VectorizedModel):
    ...
    def __init__(
        ...
        :param limit_predicted_yaw: limit predicted yaw to 0.3 * tanh(x) if enabled - recommended for more stable
            training
        """
        # In safe IL, we are learning the B-Spline coeffcients (9 values to represent future 1-s trajectory)
        # Thus, num_target = 9 in our case, while in UrbanDriver, num_target = 3 because they only predict
        # (x, y, yaw) at the next time step, num_target = 3 will also limit the queries number to 1.
        num_targets = 9  
        ## NOTE: You should adjust the dimensions of weights_scaling the same as the num_targets.
        # Example: a valid weights_scaling for num_targets = 3 could be [1, 1, 1]
        # or a valid weights_scaling for num_targets = 6 could be [0.5, 0.5, 1, 1, 1, 1]
        # Just make sure the dimensions match!
    ...
    def forward(self, data_batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for idx in range(future_num_frames):
            ...
            #### Predict next (x, y, yaw) based on predicted spline coefficients ####
            ## At prediction, this c_tensor is the output (one scene in the entire batch)
            #  we need to convert it to spline-coefficients in numpy to compute next (x, y, yaw)
            for i in range(out.shape[0]):
                # convert batch coefficients to full coefficients per scene
                c_numpy = torch.reshape(out[i, :], (3, 3))
                c_numpy = c_numpy.numpy()
                c_numpy = np.insert(c_numpy, 0, 0, axis=1)

                next_point = splev(0.1, (t, c_numpy, k))
                next_point = np.asarray(next_point, dtype=np.float32)
                next_point_tensor = torch.FloatTensor(next_point)
                next_point_tensor = torch.flatten(next_point_tensor)
                next_point_tensor = torch.unsqueeze(next_point_tensor, 0)
                next_point_batch.append(next_point_tensor)
            next_point_batch = torch.cat(next_point_batch, 0)

            #### Predict next (x, y, yaw) based on predicted spline coefficients ####

            # outputs are in ts space (optionally xy normalised)
            pred_xy_step = next_point_batch[:, 0, :2]
            pred_yaw_step = next_point_batch[:, 0, 2:3] if not self.limit_predicted_yaw else 0.3 * torch.tanh(out[:, 0, 2:3])
            ...
            #### B-Spline coefficients (ground truth) Extraction ####

            ### Motivation: 1. longer trajectories with shorter representation vectors (enhance efficiency, longer horizons)
            ### Example: To predict the future 1-s trajectory, normally we need 10 * 3 = 30 values to describe all
            ### waypoints each step, but with B-spline, we only need 9 values (see explanation below) for the same trajectory.
            ### 2. smooth, bounded trajectory within the convex hull made from control points (waypoints)

            ## Explanation Below: Assume trajectory is (x, y, yaw) = g(u)
            ## Control points: 4 local positions at ts, ts+1, ts+5, ts+10 (0s, 0.1s, 0.5s, 1.0s)
            #  drawn from data_batch["target_positions"]

            ## Control points' corresponding time: u = [0, 0.1, 0.5, 1.0]
            ## Desined knot vector: numOfKnots = numOfControlPoints (i.e. numOfCoefficients) + splineDegree (default: 3) + 1
            #  In this case, knot vector: t = [0, 0, 0, 0, 1, 1, 1, 1]
            
            ## numOfCoefficients should be 4, and each has 3 variables (x, y, yaw), thus it has 12 coefficient scalers in total
            #  However, the first coefficients at each direction are all zeros as it also starts from local (0, 0)
            #  only 12 - 3 = 9 coefficient scalers needs to be computed

            # control points extraction (x, y, yaw)
            ego_xy = data_batch["target_positions"]
            ego_yaw = data_batch["target_yaws"]

            # ground truth label for target coefficients
            target_spline_coefficients_batch = []

            # extract (:, x, y) and (:, yaw) to make an output (:, coefficients) 
            CP_ego_yaw = ego_yaw[:, [0, 4, 9]] # control points for yaws at (0.1s, 0.5s, 1.0s)
            CP_ego_yaw = np.insert(CP_ego_yaw, 0, 0, axis=1) # insert yaw (0) at 0s

            CP_ego_xy = ego_xy[:, [0, 4, 9], :] # control points for xy at (0.1s, 0.5s, 1.0s)
            CP_ego_xy = np.insert(CP_ego_xy, 0, [0, 0], axis=1) # insert xy (0, 0) at 0s

            # extract control points and compute coefficients scene-wise, 
            # then append together to obtain target coefficients
            for i in range(CP_ego_yaw.shape[0]):
                CP_ego_xy_scene_i = CP_ego_xy[i, :, :] # control points for xy at scene i
                x = CP_ego_xy_scene_i[:, 0].numpy() # convert tensor to numpy for coefficients computation
                y = CP_ego_xy_scene_i[:, 1].numpy()

                CP_ego_yaw_scene_i = CP_ego_yaw[i, :, :]
                yaw = CP_ego_yaw_scene_i[:, 0].numpy()

                (t, c, k), u = splprep((x, y, yaw), 
                                u=np.array([0, 0.1, 0.5, 1.0]),
                                s=0, k=3, task=-1, t=np.array([0, 0, 0, 0, 1.0, 1.0, 1.0, 1.0]))

                c_tensor = torch.FloatTensor(c) # convert numpy to tensor for loss computation
                c_tensor = c_tensor[:, 1:] # chop first element in each row as they are zeros (spline starts at (0, 0))
                target_spline_coefficients_batch.append(c_tensor) # append tensors together to get target coefficients

            target_spline_coefficients_batch = torch.cat(target_spline_coefficients_batch, 0) # keep dimensions to compute loss
            #### B-Spline coefficients (ground truth) Extraction ####
        ...
        output_coefficients = torch.stack(out, dim=1)
        target_coefficients = torch.stack(target_spline_coefficients_batch, dim=1)
        ...
        if self.training:
            ...
            loss = torch.mean(self.criterion(output_coefficients, target_coefficients) * target_weights)
