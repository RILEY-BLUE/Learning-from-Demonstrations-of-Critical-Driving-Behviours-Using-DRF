...
from Param_Estimation.sampling.agent_sampling_vectorized import generate_agent_sample_vectorized
...
class EgoDataset(BaseEgoDataset):
    ...
    def get_image_from_position(self, frame_idx: int, position: np.ndarray, yaw: float, 
        without_agents: Optional[bool] = False) -> np.ndarray:
        """
        Returns the objective map around a given position in a given frame.

        Args:
            frame_index (int): the frame index of the new dataset
            position (np.ndarray): the position in world frame (x, y)
            yaw (float): the yaw in the world frame # TODO

        Returns:
            an objective map with the size of the raster
        ...
        scene_index = bisect.bisect_right(self.cumulative_sizes, frame_idx)

        if scene_index == 0:
            state_index = frame_idx
        else:
            state_index = frame_idx - self.cumulative_sizes[scene_index - 1]
        ...
        return self.rasterizer.rasterize(history_frames, history_agents, history_tl_faces, position=position, yaw=yaw, 
...
class EgoDatasetVectorized(BaseEgoDataset):
    def __init__(
        ...
        """
        self.perturbation = perturbation
        self.DRF_perturbation = DRF_perturbation
    ...
    def _get_sample_function(self) -> Callable[..., dict]:
        ...
        return partial(
            ...
            rasterizer=self.rasterizer,
            DRF_perturbation=self.DRF_perturbation,
        )

    def get_scene_dataset(self, scene_index: int) -> "EgoDatasetVectorized":
        dataset = self.dataset.get_scene_dataset(scene_index)
        return EgoDatasetVectorized(self.cfg, dataset, self.vectorizer, self.rasterizer, 
                                    self.perturbation, self.DRF_perturbation)
