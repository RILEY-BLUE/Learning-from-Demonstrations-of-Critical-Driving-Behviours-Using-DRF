...
def computeVelError(preds: np.ndarray, gts: np.ndarray) -> Tuple[np.ndarray, float]:
    vel_preds = np.ravel(np.array(preds))
    vel_gts = np.ravel(np.array(gts))
    error_vels = vel_gts - vel_preds
...
scene_indices = [159] # 10 scenes for parameter identification!
...
for scene_idx in scene_indices:
    ...
    f_vel = 13.5 #np.max(smooth(np.array(temp_vel_gts), 10))
