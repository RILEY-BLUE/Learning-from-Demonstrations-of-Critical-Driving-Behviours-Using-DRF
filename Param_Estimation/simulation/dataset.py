...
from Param_Estimation.simulation.utils import disable_agents, get_frames_subset, insert_agent, insert_DRF_agent
...
class SimulationDataset:
    def __init__(self, scene_dataset_batch: Dict[int, EgoDataset], sim_cfg: SimulationConfig) -> None:
        ...
        if self.sim_cfg.disable_new_agents:
            ...
            for scene_index, dt_ego in self.scene_dataset_batch.items():
                ...
                frame_agents = self._filter_agents(scene_index, frame_agents, ego_pos)

                ego_yaw_rad = rotation33_as_yaw(frame["ego_rotation"])
                render_context = RenderContext(raster_size_px=np.array(dataset.cfg["raster_params"]["raster_size"]),
                                            pixel_size_m=np.array(dataset.cfg["raster_params"]["pixel_size"]),
                                            center_in_raster_ratio=np.array(dataset.cfg["raster_params"]["ego_center"]),
                                            set_origin_to_bottom=dataset.cfg["raster_params"]["set_origin_to_bottom"])
                raster_from_world = render_context.raster_from_world(ego_pos, ego_yaw_rad)
                frame_agents = self._filter_agents_main_road(scene_index, frame_agents, raster_from_world)
    ...
    def rasterise_gt_frame_batch(self, state_index: int) -> List[Dict[str, np.ndarray]]:
        """
        Get a frame from all scenes
    ...
    def set_agent(self, state_index: int, scene_idx: int, agent: np.ndarray) -> None:
        """Set one agent in the scene datasets.

        :param state_index: the frame index to set (same for all datasets)
        :param agents_infos: a dict mapping (scene_idx, agent_idx) to the agent array
        """
        insert_DRF_agent(agent, state_index, self.scene_dataset_batch[scene_idx].dataset)

    def rasterise_agents_frame_batch(self, state_index: int, 
                                     filter_main_road: bool) -> Dict[Tuple[int, int], Dict[str, np.ndarray]]:
        ...
        for scene_index in self.scene_dataset_batch:
            ret.update(self._rasterise_agents_frame(scene_index, state_index, filter_main_road))
        return ret

    def _rasterise_agents_frame(self, scene_index: int, state_index: int,
                                filter_main_road: bool) -> Dict[Tuple[int, int], Dict[str, np.ndarray]]:
        ...
        ego_centroid_m = frame["ego_translation"][:2]
        frame_agents = self._filter_agents(scene_index, frame_agents, ego_centroid_m)
        ego_yaw_rad = rotation33_as_yaw(frame["ego_rotation"])
        render_context = RenderContext(raster_size_px=np.array(dataset.cfg["raster_params"]["raster_size"]),
                                       pixel_size_m=np.array(dataset.cfg["raster_params"]["pixel_size"]),
                                       center_in_raster_ratio=np.array(dataset.cfg["raster_params"]["ego_center"]),
    ...
    def _filter_agents_main_road(self, scene_idx: int, frame_agents: np.ndarray,
        ...
        for idx_agent, agent in enumerate(frame_agents):
            ...
            main_road_mask[idx_agent] = agent_in_range

        return frame_agents[main_road_mask]
