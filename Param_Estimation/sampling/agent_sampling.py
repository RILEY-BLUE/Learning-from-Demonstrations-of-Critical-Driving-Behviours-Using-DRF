...
def get_agent_context(
    ...
    """
    Slice zarr or numpy arrays to get the context around the agent of interest (both in space and time)
...
def _get_relative_poses(
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]: position offsets, angle offsets, angle_raw, extent, availabilities
    ...
    yaws_rad_raw = copy.deepcopy(yaws_rad) # make a deep copy to keep raw data

    # batch transform to speed up
    positions_m = transform_points(positions_m, agent_from_world) * availabilities[:, np.newaxis]
    yaws_rad = angular_distance(yaws_rad, current_agent_yaw) * availabilities[:, np.newaxis]
    return positions_m.astype(np.float32), yaws_rad, yaws_rad_raw, extents_m, availabilities
...
def generate_agent_sample(
    ...
    # local velocities in the raster
    ...
    if selected_agent != None and len(selected_agent["velocity"]) > 0:
        result["curr_speed"] = selected_agent["velocity"][0]
    return result
